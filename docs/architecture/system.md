...
Turso/libSQL is the primary relational store. Better Auth is the sole authentication provider; `users` owns shop membership and RBAC, and the retired legacy auth endpoints have been removed. `src/lib/media-storage.ts` selects local filesystem storage outside production and Vercel Blob in production unless configured otherwise.
