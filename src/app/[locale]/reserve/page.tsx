...
export default async function ReservePage({
  ...
  for (const row of data.rows) {
    ...
    const product = productMap.get(row.product.id) ?? {
      ...
      description:
        locale === "fi" ? row.product.descriptionFi : row.product.descriptionEn,
      media: [],
      packages: [],
      dates: [],
    };
    ...
    if (
      availabilityRow &&
      !product.dates.some((item) => item.date === availabilityRow.businessDate)
    )
      product.dates.push({
        ...
        soldOut:
          availabilityRow.manualSoldOut ||
          availabilityRow.capacityMl === availabilityRow.reservedMl,
      });
  }
  for (const product of productMap.values())
    product.media = data.media
      ...
      .map((image) => ({
        id: image.id,
        url: image.url,
        alt: locale === "fi" ? image.altFi : image.altEn,
  ...
  const requestedProduct = query.product
    ...
    : undefined;
  const productAvailable = (product: PublicProduct) =>
    product.packages.some((pkg) =>
      product.dates.some(
        (date) =>
          date.acceptsOrders &&
          !date.soldOut &&
          date.remainingMl >= pkg.volumeMl,
      ),
    );
  const selectedProduct =
    requestedProduct && productAvailable(requestedProduct)
      ? requestedProduct
      : undefined;
  const requestedPackage =
    selectedProduct && query.package
      ? selectedProduct.packages.find((item) => item.id === query.package)
      : undefined;
  const packageAvailable =
    requestedPackage &&
    selectedProduct?.dates.some(
      (date) =>
        date.acceptsOrders &&
        !date.soldOut &&
        date.remainingMl >= requestedPackage.volumeMl,
    );
  const initialProductId =
    query.product === undefined ? undefined : (selectedProduct?.id ?? "");
  const initialPackageId = packageAvailable ? requestedPackage?.id : undefined;
  const notice =
    requestedProduct && !productAvailable(requestedProduct)
      ? "sold-out"
      : query.product && !requestedProduct
        ? "invalid-selection"
        : query.package && !query.product
          ? "invalid-selection"
          : requestedPackage && !packageAvailable
            ? "sold-out"
            : query.notice;
  const noticeText =
    notice === "sold-out"
      ? locale === "fi"
        ? "Tämä tuote tai pakkaus on loppuunmyyty. Valitse toinen vaihtoehto."
        : "This product or package is sold out. Please choose another option."
      : notice === "invalid-selection"
        ? locale === "fi"
          ? "Valintaa ei löytynyt. Valitse tuote alla."
    ...
    <main className="storefront">
      ...
      <header className="storefront-header">
        <div className="shell storefront-nav">
          ...
          <nav
            className="storefront-nav-links"
          ...
          >
            ...
            <a href={`/${locale}/about`}>
              {locale === "fi" ? "Meistä" : "About us"}
            </a>
          </nav>
          <MobileNav locale={locale} active="reserve" />
          <a
      ...
      <section
        className="order-section reserve-page-section"
      ...
      >
        <div className="shell">
          <div className="order-intro">
            <div>
              <p className="eyebrow">
                {locale === "fi" ? "Varaa verkossa" : "Reserve online"}
              </p>
              <h1 id="reserve-title">{t.shopHeading}</h1>
              <p className="order-intro-lede">
                {locale === "fi"
                  ? "Valitse marjat, nouto- tai toimituspäivä ja yhteystietosi."
                  : "Choose your products, fulfillment date and contact details."}
              </p>
            </div>
          ...
          {noticeText && (
            <div className="reserve-selection-notice" role="status">
              {noticeText}
            </div>
          )}
          <OrderForm
            ...
            privacyNoticeUrl={
              locale === "fi" ? "/fi/tietosuoja" : "/en/privacy"
            }
            pickup={{
              name:
                locale === "fi"
                  ? data.shop.pickupNameFi
                  : data.shop.pickupNameEn,
              address: data.shop.pickupAddress,
              instructions:
                locale === "fi"
                  ? data.shop.pickupInstructionsFi
                  : data.shop.pickupInstructionsEn,
              time: data.shop.pickupTime,
            }}
            contact={{
              phone: data.shop.contactPhone,
              email: data.shop.contactEmail,
              hours: data.shop.contactHours,
            }}
          />
        </div>
      </section>
      ...
      <footer className="storefront-footer">
        <div className="shell">
          ...
          <div className="footer-tier-bottom">
            <nav
              className="footer-inline-nav"
              aria-label={
                locale === "fi" ? "Alatunnisteen valikko" : "Footer links"
              }
            >
              <a href={`/${locale}`}>{locale === "fi" ? "Etusivu" : "Home"}</a>
              ...
              <a href={locale === "fi" ? "/fi/tietosuoja" : "/en/privacy"}>
                {locale === "fi" ? "Tietosuojaseloste" : "Privacy notice"}
              </a>
            </nav>
            <span className="footer-copy-note">
              © {new Date().getFullYear()} METSÄNILO
            </span>
          </div>
        </div>
      </footer>
