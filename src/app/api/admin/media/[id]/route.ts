...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) { try { const actor = await requirePermission(db(), request, "media.write"); const { id } = await context.params; const row = await db().select({ attachment: mediaAttachments, asset: mediaAssets }).from(mediaAttachments).innerJoin(mediaAssets, eq(mediaAssets.id, mediaAttachments.assetId)).where(and(eq(mediaAttachments.id, id), eq(mediaAttachments.shopId, env().SHOP_ID))).limit(1); if (!row[0] || !row[0].attachment.productId) throw new DomainError("NOT_FOUND", "Image not found", 404); const input = await request.json().catch(() => ({})) as { action?: string; altFi?: string; altEn?: string };
