...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await executeAdmin(request, { permission: "media.write", parse: async (incoming) => ({ id, input: await parseJson<{ action?: string; altFi?: string; altEn?: string; attachmentIds?: unknown }>(incoming) }), run: async ({ id: attachmentId, input }, { database, context: { actor, shop } }) => {
