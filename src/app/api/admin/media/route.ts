...
export async function POST(request: Request) {
  return executeAdminRoute(request, {
    ...
    parse: async (incoming) => {
      ...
      if (!(file instanceof File) || (!productId && !pageKey)) throw new DomainError("VALIDATION_ERROR", "Image file and either product or page key are required", 422);
      ...
      return { productId, pageKey, file, altFi, altEn };
    },
    run: async (input, { database, context: { actor, shop } }) => uploadAdminMedia(database, { actor, shop: { id: shop.shopId } }, input),
  });
}
...
export async function DELETE(request: Request) {
  return executeAdminRoute(request, {
    permission: "media.write",
    parse: async (incoming) => {
      const attachmentId = new URL(incoming.url).searchParams.get("attachmentId");
      if (!attachmentId) throw new DomainError("VALIDATION_ERROR", "Attachment id is required", 422);
      return attachmentId;
    },
    run: async (attachmentId, { database, context: { actor, shop } }) => deleteAdminMediaAttachment(database, { actor, shop: { id: shop.shopId } }, attachmentId),
  });
}
