...
const permissions = {
  ...
  capacity: "reports.capacity.read",
  payments: "reports.payments.read",
  customers: "reports.customers.read",
} as const;
    ...
    const result = await executeAdmin(request, {
      ...
      run: async (params, { database }) => {
        ...
        const reportFilters = {
          ...
          method: (params.get("method") as "PICKUP" | "DELIVERY" | null) ?? undefined,
          source: params.get("source") || undefined,
          outcome: params.get("outcome") || undefined,
          groupBy: (params.get("groupBy") as "day" | "week" | "month" | null) ?? "day",
        };
        ...
        const comparison = {
          from: previous.from,
          to: previous.to,
          fulfilledSalesCents: previousData.sales.fulfilledSalesCents,
          fulfilledLitresMl: previousData.sales.fulfilledLitresMl,
          fulfilledOrders: previousData.sales.fulfilledOrders,
          capacityUtilizationPercent: previousData.capacity.utilizationPercent,
        };
        if (params.get("format") === "csv") {
          return new Response(reportCsv(data, report), {
            headers: {
              "content-type": "text/csv; charset=utf-8",
              "content-disposition": `attachment; filename="metsanilo-${report}-${from}-${to}.csv"`,
            },
          });
        }
