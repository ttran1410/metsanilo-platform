import { z } from "zod";
import { addAdminDeliveryException } from "@/domain/admin-order-actions";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return executeAdminRoute(request, {
    ...
    parse: async (incoming) => {
      ...
      if (!parsed.success) throw new DomainError("VALIDATION_ERROR", "Invalid delivery exception", 422);
      return parsed.data;
    },
    run: async (input, { database, context: { actor, shop } }) =>
      addAdminDeliveryException(database, { actor, shop: { id: shop.shopId } }, { orderId: (await params).id, ...input }),
  });
}
