...
const command = z.object({ productId: z.string(), packageId: z.string(), quantity: z.number().int().min(1).max(100), fulfillmentDate: z.string(), fulfillmentMethod: z.enum(["PICKUP", "DELIVERY"]), customerName: z.string(), mobile: z.string(), email: z.string().optional(), streetAddress: z.string().optional(), postalCode: z.string().optional(), city: z.string().optional(), itemSubtotalCents: z.number().int().nonnegative().optional(), deliveryFeeCents: z.number().int().nonnegative().optional(), completedStatus: z.enum(["PICKED_UP", "DELIVERED"]), completedAt: z.string(), source: z.enum(["PHONE", "OTHER"]), reason: z.string().min(2), paymentAmountCents: z.number().int().positive().optional() });
