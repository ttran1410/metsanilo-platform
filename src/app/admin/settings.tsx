...
export function OperationsSettings() {
  ...
  return <section className="shell pb-10"><h2 className="text-2xl font-bold">Operational settings</h2><p className="mt-1 text-sm">Enable payment methods and maintain the customer-facing contact details.</p>{message && <p className="card mt-3" role="status">{message}</p>}<div className="card mt-3 grid gap-2 md:grid-cols-3">{methods.map((method) => <label className="flex items-center gap-2" key={method.method}><input type="checkbox" checked={method.enabled} onChange={() => void toggle(method)} />{method.method.replace("_", " ")}</label>)}</div><form className="card mt-3 grid gap-3 md:grid-cols-3" onSubmit={saveContact}><h3 className="font-bold md:col-span-3">Customer contact</h3><label className="field"><span>Phone</span><input name="phone" type="tel" value={contact.phone} onChange={(event) => setContact({ ...contact, phone: event.target.value })} placeholder="+358 44 951 2904" /></label><label className="field"><span>Email</span><input name="email" type="email" value={contact.email} onChange={(event) => setContact({ ...contact, email: event.target.value })} placeholder="contact@example.fi" /></label><label className="field"><span>Contact hours</span><input name="hours" value={contact.hours} onChange={(event) => setContact({ ...contact, hours: event.target.value })} placeholder="08:00–20:00 Mon–Fri" /></label><button className="btn md:col-span-3" type="submit">Save contact details</button></form></section>;
}
