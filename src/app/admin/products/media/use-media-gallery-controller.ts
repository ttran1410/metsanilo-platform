...
export function useMediaGalleryController({ productId, onRefresh }: { productId: string; onRefresh: () => void }) {
  ...
  function setPrimary(image: AdminProductMedia) { if (image.attachmentId) void send(`/api/admin/media/${image.attachmentId}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "primary" }) }, "Could not set primary hero image.", "Primary hero image updated."); }
  async function saveAlt(event: FormEvent<HTMLFormElement>, image: AdminProductMedia) { event.preventDefault(); if (!image.attachmentId) return; const values = new FormData(event.currentTarget); await send(`/api/admin/media/${image.attachmentId}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "metadata", altFi: values.get("altFi"), altEn: values.get("altEn") }) }, "Could not save alt tags.", "Alt text tags updated."); setAltEditingId(null); }
  ...
  function reorderMedia(ordered: AdminProductMedia[]) { const ids = ordered.map((item) => item.attachmentId).filter((id): id is string => Boolean(id)); if (ids.length) void send(`/api/admin/media/${ids[0]}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "reorder", attachmentIds: ids }) }, "Could not reorder gallery.", "Gallery order updated."); }
