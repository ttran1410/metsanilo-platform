...
export function UserModule() {
  ...
  async function load() { const response = await fetch("/api/admin/users"); const body = await response.json(); if (response.ok) setUsers(body.data); else setMessage(body.code ?? "Permission denied"); }
  ...
  async function resetPassword(user: User) { const response = await fetch(`/api/admin/users/${user.id}/password`, { method: "POST" }); const body = await response.json(); if (!response.ok) return setMessage(body.message ?? "Password reset failed"); window.alert(`Temporary password for ${user.email}:\n\n${body.data.temporaryPassword}\n\nCopy it now. It cannot be recovered later.`); }
