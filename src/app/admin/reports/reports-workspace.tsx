...
export function ReportsWorkspace({ permissions }: { permissions: Record<ReportKey, boolean> }) {
  ...
  useEffect(() => { void fetch("/api/admin/products", { cache: "no-store" }).then((response) => response.json()).then((body) => setProducts((body.data ?? []).map((product: { id: string; nameEn: string }) => ({ id: product.id, nameEn: product.nameEn })))).catch(() => undefined); }, []);
  async function load() {
...
function delta(current: number, previous: number, suffix = "") { if (!previous) return "new"; const change = Math.round(((current - previous) / Math.abs(previous)) * 100); return `${change >= 0 ? "+" : ""}${change}%${suffix ? ` ${suffix}` : ""}`; }
