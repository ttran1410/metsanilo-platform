...
export function DashboardModule() {
  ...
  return <section id="dashboard" className="shell py-6">
    ...
    <div className="dashboard-grid mt-5"><section className="card"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xl font-bold">Capacity · next 14 days</h3><p className="text-sm text-slate-600">Reserved versus available litres by product.</p></div><Link className="btn btn-secondary" href="/admin/availability">Manage</Link></div><div className="mt-4 grid gap-3">{data.capacity.map((row) => <div className="capacity-row" key={`${row.productNameFi}-${row.date}`}><div className="flex justify-between gap-3 text-sm"><strong>{row.productNameFi}</strong><span className={row.soldOut ? "pill pill-warning" : "pill"}>{row.soldOut ? "Sold out" : row.date}</span></div><div className="capacity-track" role="progressbar" aria-label={`${row.productNameFi} ${row.date}`} aria-valuenow={row.reservedLitres} aria-valuemin={0} aria-valuemax={row.capacityLitres}><span style={{ width: `${Math.min(100, row.reservedLitres / Math.max(row.capacityLitres, 1) * 100)}%` }} /></div><p className="text-xs text-slate-600">{row.reservedLitres} L reserved · {row.remainingLitres} L remaining</p></div>)}</div></section>
