...
export function AdminNavigation({ role, displayName, email, items }: { role: Role; displayName: string; email: string | null; items: NavItem[] }) {
  ...
  return <header className="admin-shell-header">
    <div className="admin-shell-brand">
      ...
      <div className="admin-header-actions"><div className="admin-quick-tools" aria-label="Operations shortcuts"><Link className="admin-quick-tool" href="/admin/orders" aria-label="Open order search"><span aria-hidden="true">⌕</span><span>Quick search</span><kbd>⌘K</kbd></Link><Link className="admin-quick-tool" href="/admin" aria-label="Open operations alerts"><span aria-hidden="true">◌</span><span>Alerts</span></Link></div><span className="admin-user-state"><i aria-hidden="true" />{role}</span><AdminUserMenu displayName={displayName} email={email} role={role} /></div>
