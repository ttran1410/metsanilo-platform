...
export default async function EditOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const products = await listManagerProducts(db());
    const availabilityList = await listManagerAvailability(db());
    return (
      <AdminRouteFrame permission="orders.update">
        <main className="shell admin-profile-page">
          <div className="admin-page-heading">
            <a className="back-link" href={`/admin/orders/${id}`}>← Order detail</a>
