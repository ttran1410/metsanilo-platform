...
import { getAdminOrderDetail } from "@/domain/admin-order-actions";
...
export default async function OrderDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ updated?: string }> }) {
  ...
  let detail; try { detail = await getAdminOrderDetail(db(), { actor, shop: { id: env().SHOP_ID } }, id); } catch { return <AdminRouteFrame><AdminNotFoundState title="Order Not Found" description={`No order matching ID or reference "${id}" was found in database.`} backHref="/admin/orders" backLabel="← Return to orders list" /></AdminRouteFrame>; }
