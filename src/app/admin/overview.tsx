...
export function AdminOverview({ initialData }: { initialData?: DashboardData }) {
  ...
  return (
    <main className="admin-overview">
      ...
      {(hasOrderAttention || hasUnreadAlerts || notice || error) && <section className={`admin-overview-alert${error ? " is-error" : ""}`} aria-live="polite"><div><strong>{error || notice || (hasOrderAttention ? `${data.attentionCount} order${data.attentionCount === 1 ? "" : "s"} need attention` : `${data.unreadNotifications} unread team alert${data.unreadNotifications === 1 ? "" : "s"}`)}</strong><span>{error ? "Resolve the issue, then refresh the overview." : data.overdueNew.length > 0 ? `${data.overdueNew.length} new order(s) have been waiting over 15 minutes.` : data.unconfirmedDeliveryCount > 0 ? `${data.unconfirmedDeliveryCount} delivery address(es) need confirmation.` : hasUnreadAlerts ? "Open Alerts in the top bar to review team notifications." : "Operations are moving normally."}</span></div>{hasOrderAttention ? <Link href="/admin/orders?view=triage">Review orders</Link> : hasUnreadAlerts ? <span className="admin-overview-alert-hint">Use Alerts above</span> : null}</section>}
      <section className="admin-overview-section"><div className="admin-overview-heading"><div><p className="eyebrow">The day&apos;s pick list</p><h2>From message to crate</h2></div><Link href="/admin/orders">See all orders ↗</Link></div><div className="admin-overview-spine">{stages.map(([key, label, detail, status], index) => { const stage = data.funnel[key as keyof DashboardData["funnel"]]; const stageStatus = key === "ready" ? "READY_STAGE" : key === "done" ? "FULFILLED" : status; return <Link className={`admin-overview-stage${key === "packing" ? " is-current" : ""}`} href={`/admin/orders?view=TODAY&status=${stageStatus}`} key={key}><div><span>{label}</span><small>{String(index + 1).padStart(2, "0")}</small></div><strong>{stage.count}</strong><em>{stage.volumeLitres.toFixed(1)} L</em><small>{detail}</small>{index < stages.length - 1 && <i aria-hidden="true" />}</Link>; })}</div></section>
      ...
      {data.overdueNew.length > 0 && <section className="admin-overview-section"><div className="admin-overview-heading"><div><p className="eyebrow">Human replies first</p><h2>New orders</h2><p className="muted">Incoming orders waiting for confirmation.</p></div><Link href="/admin/orders?view=NEEDS_CONFIRMATION">Open new orders ↗</Link></div><div className="admin-overview-queue">{data.overdueNew.slice(0, 3).map((order) => <article key={order.id}><div><strong>{order.publicReference}</strong><span>{order.customerName} · waiting {order.ageMinutes} min</span></div><div><AdminStatusBadge status="NEW" /><button className="btn" type="button" onClick={() => void quickConfirm(order.id, order.publicReference, order.version)}>Quick confirm</button></div></article>)}</div></section>}
    </main>
  );
}
