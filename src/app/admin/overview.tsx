...
const stages = [
  ["intake", "New", "needs a reply", "NEW"],
  ["confirm", "Confirmed", "in the plan", "CONFIRMED"],
  ["packing", "Picking", "at the shed", "PICKING"],
  ["ready", "Ready", "to hand over", "READY"],
  ["done", "Done", "out the door", "DELIVERED"],
] as const;
...
export function AdminOverview({ initialData }: { initialData?: DashboardData }) {
  ...
  return (
    <main className="admin-overview">
      <section className="admin-overview-intro"><div><p className="eyebrow">{data.businessDate} · harvest day</p><h1>The shed is<br /><em>in motion.</em></h1><p>A live read on what needs your hands next. Capacity, orders, and handovers in one calm view.</p></div><div className="admin-overview-clock"><span>Last synced</span><strong>{new Date(data.asOf).toLocaleTimeString("fi-FI", { hour: "2-digit", minute: "2-digit" })}</strong><small>{refreshing ? "Refreshing…" : "Live from operations"}</small><button className="btn btn-secondary" type="button" disabled={refreshing} onClick={() => void load()}>{refreshing ? "Refreshing…" : "Refresh"}</button></div></section>
      {(totalExceptions > 0 || notice || error) && <section className={`admin-overview-alert${error ? " is-error" : ""}`} aria-live="polite"><div><strong>{error || notice || `${totalExceptions} order${totalExceptions === 1 ? "" : "s"} need attention`}</strong><span>{error ? "Resolve the issue, then refresh the overview." : data.overdueNew.length > 0 ? `${data.overdueNew.length} new order(s) have been waiting over 15 minutes.` : data.unconfirmedDeliveryCount > 0 ? `${data.unconfirmedDeliveryCount} delivery address(es) need confirmation.` : data.unreadNotifications > 0 ? `${data.unreadNotifications} unread team alert(s) remain.` : "Operations are moving normally."}</span></div><Link href="/admin/orders?view=triage">Open triage</Link></section>}
      <section className="admin-overview-section"><div className="admin-overview-heading"><div><p className="eyebrow">The day&apos;s pick list</p><h2>From message to crate</h2></div><Link href="/admin/orders">See all orders ↗</Link></div><div className="admin-overview-spine">{stages.map(([key, label, detail, status], index) => { const stage = data.funnel[key as keyof DashboardData["funnel"]]; return <Link className={`admin-overview-stage${key === "packing" ? " is-current" : ""}`} href={`/admin/orders?view=TODAY&status=${status}`} key={key}><div><span>{label}</span><small>{String(index + 1).padStart(2, "0")}</small></div><strong>{stage.count}</strong><em>{stage.volumeLitres.toFixed(1)} L</em><small>{detail}</small>{index < stages.length - 1 && <i aria-hidden="true" />}</Link>; })}</div></section>
      <div className="admin-overview-grid"><section className="admin-overview-panel"><div className="admin-overview-heading"><div><p className="eyebrow">What the van can carry</p><h2>Today&apos;s capacity</h2></div><Link href="/admin/availability">Manage ↗</Link></div><div className="admin-overview-capacity"><strong>{data.volume.percentage}<span>%</span></strong><div><span>{data.volume.reservedLitres} L reserved</span><small>of {data.volume.capacityLitres} L available</small></div></div><div className="admin-overview-bar"><span style={{ width: `${Math.min(100, Math.max(0, data.volume.percentage))}%` }} /></div><div className="admin-overview-splits"><span><i className="pickup" />Pori stall <b>{data.volume.pickupVolumeLitres.toFixed(1)} L</b></span><span><i className="delivery" />Home delivery <b>{data.volume.deliveryVolumeLitres.toFixed(1)} L</b></span></div></section><section className="admin-overview-panel"><div className="admin-overview-heading"><div><p className="eyebrow">Cash position</p><h2>Today&apos;s revenue</h2></div><span className="admin-overview-live-dot"><i />Live</span></div><dl className="admin-overview-financials"><div><dt>Booked</dt><dd>{formatAdminMoney(data.financials.grossBookedCents)}</dd></div><div className="success"><dt>Collected</dt><dd>{formatAdminMoney(data.financials.collectedCents)} <small>{data.financials.collectedPercentage}%</small></dd></div><div className="warning"><dt>Due at handover</dt><dd>{formatAdminMoney(data.financials.outstandingCents)}</dd></div></dl><button className="btn btn-secondary admin-overview-automation" type="button" disabled={runningAutomation} onClick={() => void runAutomation()}>{runningAutomation ? "Running automation…" : "Run automation now"}</button></section></div>
      <section className="admin-overview-section"><div className="admin-overview-heading"><div><p className="eyebrow">Harvest forecast</p><h2>Next three days</h2></div><Link href="/admin/availability?view=WEEK">Open calendar ↗</Link></div><div className="admin-overview-lookahead">{data.lookahead.map((day) => <Link href="/admin/availability" className="admin-overview-day" key={day.date}><span>{day.label}</span><strong>{day.reservedLitres} L <small>of {day.capacityLitres} L</small></strong><div className="admin-overview-mini-bar"><i style={{ width: `${Math.min(100, day.percentage)}%` }} /></div><small>{day.remainingLitres} L remaining · {day.percentage}% booked</small></Link>)}</div></section>
      {data.overdueNew.length > 0 && <section className="admin-overview-section"><div className="admin-overview-heading"><div><p className="eyebrow">Human replies first</p><h2>Orders waiting for confirmation</h2></div><Link href="/admin/orders?view=NEEDS_CONFIRMATION">Open queue ↗</Link></div><div className="admin-overview-queue">{data.overdueNew.slice(0, 3).map((order) => <article key={order.id}><div><strong>{order.publicReference}</strong><span>{order.customerName} · waiting {order.ageMinutes} min</span></div><div><AdminStatusBadge status="NEW" /><button className="btn" type="button" onClick={() => void quickConfirm(order.id, order.publicReference, order.version)}>Quick confirm</button></div></article>)}</div></section>}
    </main>
  );
}
