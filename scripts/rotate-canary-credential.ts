...
export async function rotateCanaryCredential(
  ...
  const executeRotation = async (tx: DatabaseExecutor): Promise<RotateCanaryResult> => {
    ...
    const randomPasswordPlaintext = randomBytes(48).toString("base64url");
    const newHash = hashPassword(randomPasswordPlaintext);
    ...
    return {
      ...
      userId: user.id,
      email: user.email || "",
      newPasswordPlaintext: randomPasswordPlaintext,
      rotatedAt,
      auditId,
    };
  };
