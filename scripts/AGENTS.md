...
- `audit:auth` (`scripts/audit-auth-readiness.ts`) runs database readiness verification before cutovers (strictly requiring active ADMIN and MANAGER role coverage, 100% active user credential coverage, and bidirectional orphan checks).
- `smoke:auth` (`scripts/auth-readiness-smoke.ts`) executes live HTTP authentication smoke tests against a specified target URL (`AUTH_SMOKE_BASE_URL`), protecting against unintended execution on the canonical production origin unless `AUTH_SMOKE_ALLOW_PRODUCTION=true` is set. Manager credentials (`AUTH_SMOKE_MANAGER_EMAIL` / `AUTH_SMOKE_MANAGER_PASSWORD`) are optional for local runs but must both be provided during formal staging/production pre-cutover smoke validation.
